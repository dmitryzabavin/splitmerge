...
class splitmerge:
    ...
    def get_centroid_distances(self):
        ...
        shortest_distances = pd.DataFrame({'distance': shortest_distances, 'approx_r': shortest_distances / 2})
        ...
        all_relations.loc[:, 'neighbor_approx_r'] = get_attribute_value(all_relations,
                                                                        all_relations.loc[:, 'closest'],
                                                                        'approx_r', 'id')
        all_relations.loc[:, 'approx_lambda'] = all_relations.loc[:, 'distance'] -\
                                                all_relations.loc[:, 'approx_r'] -\
                                                all_relations.loc[:, 'neighbor_approx_r']

        all_relations.index = [i for i in range(all_relations.shape[0])]
        return all_relations

    def cluster_merge(self, order_by='approx_lambda'):
    ...
    def measure_steps(self, key='approx_lambda'):
...
# ONE OTHER WAY TO EVALUATE THE BEST VALUE OF approx_lambda FOR CLUSTER MERGE
    def two_distance_clusters(self):
        centroid_distances = self.stepwise_merge
        clusterer = KMeans(n_clusters=2, random_state=0).fit(pd.DataFrame(centroid_distances.loc[:, 'approx_lambda']))
        centroid_distances.loc[:, 'label'] = clusterer.labels_
        return centroid_distances.groupby('label')['n_clusters'].max().min()


    def __init__(self, source_clustering=None, centroids=None, volumes=None):
        if (source_clustering is None) and (centroids is None):
            ...
            self.stepwise_merge, self.stepwise_label_map = self.cluster_merge('approx_lambda')
            # print(self.stepwise_merge)
            # print(self.stepwise_label_map)
            self.k_between_two_distance_clusters = self.two_distance_clusters()
            self.steps = self.measure_steps('approx_lambda')
            self.n_merged = None
...
# PLOT DEPENDENCY BETWEEN approx_lambda AND n_clusters - RESULTS OF MERGE
# import matplotlib.pyplot as plt
# plt.plot(approx_lambda.loc[:, 'approx_lambda'], approx_lambda.loc[:, 'n_clusters'], marker='s')
